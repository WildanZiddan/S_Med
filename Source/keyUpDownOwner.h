...
//    BorderSwitchRole(x, y, lebar);
    ...
    if(count == 1){
        SetColorBlock(0,12);
        gotoprinttext(138,19,"U B A H  N A M A");
    }
    while (1) {
        ch = getch();
        if (ch == ENTER || ch == TAB) {
...
//                BorderSwitchRole(x, y, lebar);
            ...
            if(count == 1){
                ...
                SetColorBlock(0,12);
                gotoprinttext(140,21,"     K E M B A L I ");
            }
        } else if (ch == DWN_KEY) {
            if (count < 4) {
                Beep(1000, 100);
...
void SpecialkeyMenuDataBeli(int x, int y, int lebar,int tinggi, int *output) {
        //Declaration
        int count = 1;
        char ch;
